...
class ExKmallocTester(BaseModuleTester):
    ...
    def test_parameter_access(self):
        ...
        # Test setting parameter to abother value
        test_value = 8
    ...
    def test_allocation_success(self):
        ...
        success = self.assert_dmesg_contains(
            r"kmalloc: SUCCESS", "Memory allocation successful"
        )

        # Check for allocation details
        if success:
            self.assert_dmesg_contains(
                r"kmalloc: \d+ byte, \d+ ns, type: PHYSICALLY_CONTIGUOUS",
    ...
    def parse_allocation_stats(self, dmesg_output, requested_kb):
        ...
        success_pattern = rf"kmalloc: ({requested_kb}|\d+) byte.*kmalloc: SUCCESS.*kmalloc: (\d+) byte, (\d+) ns"
    ...
    def test_allocation_with_different_sizes(self):
        ...
        while size <= 1024 * 1024:  # 1024 MB in KB
        ...
        for size in test_sizes:
            ...
            self.assert_dmesg_contains(
                rf"kmalloc: {size * 1024} byte", f"Allocation with size {size} KB"
            )
    ...
    def run_all_tests(self):
        """Run all tests for ex_kmalloc module"""
