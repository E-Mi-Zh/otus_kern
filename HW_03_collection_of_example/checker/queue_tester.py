...
class QueueModuleTester(BaseModuleTester):
    def test_queue_operations(self):
        ...
        try:
            ...
            self.assert_dmesg_contains(r"Enqueued: 10", "Enqueue item 10")

            self.set_parameter("value", 20)
            self.set_parameter("cmd", "enqueue")
            self.assert_dmesg_contains(r"Enqueued: 20", "Enqueue item 20")

            # Test peek operation
            print("\n--- Peek Operation ---")
            self.set_parameter("cmd", "peek")
            self.assert_dmesg_contains(
                r"Front item: 10", "Peek should show first item 10"
            )
            ...
            self.assert_dmesg_contains(
                r"Queue contents.*10 20", "Queue should contain [10, 20]"
            )
            ...
            self.assert_dmesg_contains(
                r"Sum of values.*30", "Sum of values should be equal 30"
            )
            ...
            self.assert_dmesg_contains(
                r"Dequeued: 10", "Dequeue should return first item 10"
            )
            ...
            self.set_parameter("cmd", "total")
            self.assert_dmesg_contains(
                r"Sum of values.*20", "Sum of values should be equal 20"
            )

            # Test clear operation
            print("\n--- Clear Operation ---")
            self.set_parameter("cmd", "clear")
            self.assert_dmesg_contains(r"Queue cleared", "Clear queue")
