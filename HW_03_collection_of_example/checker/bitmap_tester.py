...
class BitmapModuleTester(BaseModuleTester):
    def test_bitmap_operations(self):
        ...
        try:
            ...
            self.assert_dmesg_contains(
                r"Current bitmap: 1[01]{4}1[0]+",
                "Verify bitmap contents [0,5 set]"
            )
            ...
            self.assert_dmesg_contains(
                r"First set bit found at position 0",
                "Find first set bit"
            )

            self.set_parameter("cmd", "find_zero")
            self.assert_dmesg_contains(
                r"First zero bit found at position 2",
                "Find first zero bit"
            )
